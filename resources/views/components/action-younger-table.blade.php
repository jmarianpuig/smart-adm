    ...
    <div class="flex space-x-1">
        ...
        @if ($data->imageables[1]->imageable_type === 'App\Models\Actor')
            @php
                ...
                $showRoute = route('youngers.actors.show', compact('data'));
                $editRoute = route('youngers.actors.edit', compact('data'));
                $pdfRoute = route('exportsYounger.pdf', compact('modelType', 'info'));
                $excelRoute = route('exportsYounger.excel', compact('modelType', 'info'));
                $moveToRoute = route('migrate.actorToExtra', $data->id);
                $removeRoute = route('youngers.actors.remove', compact('data'));
                $restoreRoute = route('youngers.actors.restore', compact('data'));
        ...
        @if ($data->imageables[1]->imageable_type === 'App\Models\Xtra')
            @php
                ...
                $showRoute = route('youngers.extras.show', compact('data'));
                $editRoute = route('youngers.extras.edit', compact('data'));
                $pdfRoute = route('exportsYounger.pdf', compact('modelType', 'info'));
                $excelRoute = route('exportsYounger.excel', compact('modelType', 'info'));
                $moveToRoute = route('migrate.extraToActor', $data->id);
                $removeRoute = route('youngers.extras.remove', compact('data'));
                $restoreRoute = route('youngers.extras.restore', compact('data'));
