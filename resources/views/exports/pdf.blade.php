...
<body>
    ...
    <div class="max-image-container my-3">
        <img src="{{ $url . $info->imageables[1]->url }}"
            class="max-image">
    </div>
    <div class="max-image-container my-3">
        <img src="{{ $url . $info->imageables[2]->url }}"
            class="max-image">
    </div>
    <div class="mt-4 details">
        <p><strong>Nombre:</strong> <span
                id="nombreCompleto">{{ $info->full_name }}</span>
        </p>
        <p><strong>N. Artístico:</strong> <span
                id="nombreAlias">{{ $info->FormattedAlias }}</span>
        </p>
        ...
        <p><strong>Url Videobook:</strong> <span><a href="{{ $info->url_book }}" target="_blank" style="text-decoration: none; {{ $info->url_book ? 'color:green' : 'color:red' }}" >{{ $info->url_book ?? 'No dispone'}}</a></span></p>
    </div>
    @livewireScripts
</body>
