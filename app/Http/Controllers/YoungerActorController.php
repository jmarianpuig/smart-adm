...
use App\Http\Requests\MenorActorUpdateRequest;
...
class YoungerActorController extends Controller
{
    ...
    public function __construct(WebUser $webUser, Actor $actor)
    {
        ...
        $this->middleware('permission:youngers.actors.index', ['only' => ['index']]);
        $this->middleware('permission:youngers.actors.show', ['only' => ['show']]);
        $this->middleware('permission:youngers.actors.edit', ['only' => ['edit', 'update']]);
        $this->middleware('permission:youngers.actors.create', ['only' => ['create', 'store']]);
        $this->middleware('permission:youngers.actors.update', ['only' => ['update']]);
        $this->middleware('permission:youngers.actors.remove', ['only' => ['removeYoungerActorFromList']]);
        $this->middleware('permission:youngers.actors.restore', ['only' => ['restoreYoungerActorToList']]);

        $this->middleware('permission:youngers.actors.destroy', ['only' => ['destroy']]);
    }

    public function index(): View
    {
        $modelType = 'Actor';
        return view('youngers.actors.index', compact('modelType'));
    }

    public function show(Actor $data): View
    {
        return view('youngers.actors.show', compact('data'));
    }

    public function edit(Actor $data): View
    {
        return view('youngers.actors.edit', compact('data'));
    }

    public function update(MenorActorUpdateRequest $request, $actorId)
    {
        ...
        $parents = ParentDetail::where('user_id', $actor->user_id)->first();
        ...
        try {
            if ($webUser && $actor && ($webUser->id == $actor->user_id)) {
                ...
                if ($parents) {
                    ...
                    $this->updateParent($request, $actor);
                } else {
                    $this->createParentDetails($request, $actor);
                }
                ...
                return redirect()->route('youngers.actors.show', $actor->slug);
            } else {
    ...
    private function updateActor(MenorActorUpdateRequest $request, $actor)
    {
        $full_name = $request->input('name') . ' ' . $request->input('first_lname') . ' ' . $request->input('second_lname');
    ...
    private function createParentDetails(MenorActorUpdateRequest $request, $actor)
    {
    ...
    private function updateParent(MenorActorUpdateRequest $request, $actor)
    {
        ...
        try {
            ...
            $parents = ParentDetail::where('user_id', $actor->user->id)->first();

            // nombre completo
            $full_name = $request->input('parents_name') . ' ' . $request->input('parents_first_lname') . ' ' . $request->input('parents_second_lname');

            //update del modelo ParentDtail
            $parents->update([
                'name' => $request->input('parents_name'),
                'first_lname' => $request->input('parents_first_lname'),
                'second_lname' => $request->input('parents_second_lname'),
    ...
    protected function removeYoungerActorFromList(Actor $data)
    {
        try {
            $data->update([
                "removed" => true
            ...
            return redirect()->route('youngers.actors.index');
        } catch (\Exception $e) {
    ...
    protected function restoreYoungerActorToList(Actor $data)
    {
        try {
            $data->update([
                "removed" => false
