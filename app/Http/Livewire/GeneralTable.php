...
class GeneralTable extends DataTableComponent
{
    protected $model;
    ...
    public function builder(): Builder
    {
        if ($this->type === 'Xtra') {
            ...
            // Manejo de error o lógica predeterminada si no se especifica el tipo de modelo
            // Para usar con mas modelos si lo tengo
            abort(403);
        }
    }
    ...
    public function columns(): array
    {
        return [
            ...
            BooleanColumn::make('Discap.', 'is_disabled')
                ...
                ->deselected()
                ->hideIf(true),
            BooleanColumn::make('Jubilado/a', 'is_retired')
                ->sortable()
                ->collapseOnTablet()
                ->deselected()
                ->hideIf(true),
    ...
    public function filters(): array
    {
        $filters = [
            ...
            SelectFilter::make('¿Tiene Tattoos?')
                ->options([
                    '' => 'Todos',
                    '1' => 'Sí',
                    '0' => 'No',
                ...
                ->filter(function (Builder $builder, string $value) {
                    if ($value === '1') {
                        $builder->where('has_tattoos', true);
                    } elseif ($value === '0') {
                        $builder->where('has_tattoos', false);
                    }
                }),
